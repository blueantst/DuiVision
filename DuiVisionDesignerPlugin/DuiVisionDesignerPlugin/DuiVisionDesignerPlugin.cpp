// DuiVisionDesignerPlugin.cpp : Defines the initialization routines for the DLL.
//
...
static AFX_EXTENSION_MODULE DuiVisionDesignerPluginDLL = { NULL, NULL };

HINSTANCE GetDllInstance()
{
	return DuiVisionDesignerPluginDLL.hModule;
}
...
DllMain(HINSTANCE hInstance, DWORD dwReason, LPVOID lpReserved)
{
	...
	{
		TRACE0("DuiVisionDesignerPlugin.DLL Initializing!\n");
		
		// Extension DLL one-time initialization
		if (!AfxInitExtensionModule(DuiVisionDesignerPluginDLL, hInstance))
			return 0;
		...
		new CDynLinkLibrary(DuiVisionDesignerPluginDLL);
	}
	else if (dwReason == DLL_PROCESS_DETACH)
	{
		TRACE0("DuiVisionDesignerPlugin.DLL Terminating!\n");
		// Terminate the library before destructors are called
		AfxTermExtensionModule(DuiVisionDesignerPluginDLL);
	}
	return 1;   // ok
}
